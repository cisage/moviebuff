...
exports.updateList = async (req, res, next) => {
    try {
        ...
        if (!list)
            return next(new AppError('This movie list does not exist',401))
        const newObj = {}
        if (req.body.title)
...
exports.deleteList = async (req,res,next) => {
    try {
        ...
        if (!list)
            return next(new AppError('This list does not exist'), 401)
        ...
        if (categoryQuery) {
            if (genreQuery) {
                lists = await list.aggregate([
                    { $sample: { size: 10 } },
                    { $match: { category: categoryQuery, genre: genreQuery}},
                ])
            }
        }
